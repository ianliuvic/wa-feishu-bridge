...
- Verify deployment with the full rendered-content version marker, not only the product-list fingerprint. Pass that content version as a query parameter on Zoho's `content_url` so Zoho cannot reuse a stale imported HTML snapshot.
- If the repository points to a campaign that Zoho reports as missing or deleted, treat the key as stale and create exactly one replacement draft.
