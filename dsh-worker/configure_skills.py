...
def write_wearhongxiu() -> str:
    ...
    temporary = WEAR_CONFIG.with_suffix(".tmp")
    descriptor = os.open(temporary, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600)
    with os.fdopen(descriptor, "w", encoding="utf-8") as handle:
        handle.write("# Generated by dsh-worker configure_skills.py from the\n")
        handle.write("# Coolify environment. Do not edit: it is rebuilt on every start.\n")
        for key, value in present.items():
            # A missing optional value is written empty rather than dropped, so
            # the file always shows the full expected shape.
            handle.write(f"{key}={value}\n")
    temporary.replace(WEAR_CONFIG)
    os.chmod(WEAR_CONFIG, 0o600)

    configured = [k for k, v in present.items() if v.strip()]
    return f"wrote {WEAR_CONFIG} with {len(configured)}/{len(WEAR_KEYS)} values set"
